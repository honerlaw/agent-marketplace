...
def test_http_mode_serves_with_uvicorn(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    env = {"OPENAI_ADS_API_KEY": "sk-test", "MCP_TRANSPORT": "http", "MCP_AUTH_TOKEN": "t"}
...
def test_module_entry_point_calls_main(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    runpy.run_module("openai_ads_mcp", run_name="__main__")
    assert calls == [True]
...
async def test_stdio_subprocess_speaks_clean_mcp() -> None:
    parameters = StdioServerParameters(
        ...
        env={"OPENAI_ADS_API_KEY": "sk-test", "OPENAI_ADS_OPENAPI_PATH": str(FIXTURE_SPEC)},
    )
    async with Client(parameters) as client:
        listed = await client.list_tools()
        endpoints = await client.call_tool("openai_ads_list_endpoints", {"text_filter": "campaign"})
    assert len(listed.tools) == 4
    assert endpoints.structured_content == {
        "result": ["GET /campaigns — List Campaigns", "POST /campaigns — Create Campaign"]
    }
