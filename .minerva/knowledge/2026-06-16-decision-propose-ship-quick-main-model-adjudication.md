...
The mechanism that distinguishes `-quick` is a **fail-closed escalation predicate** that is the *structural inverse* of `-auto`'s skip predicate (see [[2026-05-31-decision-per-decision-skip-over-sizing-gate]]):
...
- New orchestrator-style skills that share the lifecycle clone it standalone (per the rejected-alternatives above and [[2026-06-06-pattern-rejected-alternative-reinvented-at-runtime]]), rather than sharing references or adding mode flags.
