...
- Uncatalogued entries and un-reciprocated forward links are **warnings**, so a branch's
  ...
  [[053-constraint-reconciliation-state-is-not-a-scalar]] for why that is not gated on
  the watermark.
- Removing the conflicts removed the only thing incidentally catching duplicate entry
  numbers — which is why [[055-constraint-knowledge-allocation-scans-across-branches]]
