...
**Date**: 2026-07-27
**Type**: constraint
**Context**: .minerva/work/047-pin-agent-dispatch-sync
...
- This is the same defect shape as [[007-constraint-skills-must-call-tools-not-prose]] and [[049-constraint-handoffs-name-skill-tool]] one level down: the text names the right tool but omits the parameter that makes it behave the way the surrounding protocol assumes. The general rule those three share: **a skill instruction must pin every argument the protocol's next step depends on** — naming the tool is not enough when a default silently changes the control flow.
- Per [[030-pattern-rejected-alternative-reinvented-at-runtime]], the convention is mechanized rather than only recorded here; a knowledge entry alone would not have stopped the recurrence.

## Related
- [[007-constraint-skills-must-call-tools-not-prose]] — builds on
- [[049-constraint-handoffs-name-skill-tool]] — builds on
