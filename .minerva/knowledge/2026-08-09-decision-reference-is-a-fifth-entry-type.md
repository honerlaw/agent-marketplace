...
- [[2026-08-09-pattern-read-authored-metadata-from-where-it-is]] — the sibling: there the parser could not read a type the entry did state, here the vocabulary had no slot for one four entries did state. Both are the tooling's model of the corpus lagging the corpus
- [[2026-06-02-decision-knowledge-wiki-navigability-layer]] — the index this adds a section to, and the navigability argument that makes an uncatalogued entry a real loss
