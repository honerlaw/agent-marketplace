...
`minerva:lint` ([[2026-06-03-decision-minerva-lint-read-only]]) — which applies the
...
- **Entry edits** (the missing-reciprocal fix, which adds a `## Related` line and/or
  ...
  ([[2026-06-02-constraint-promote-narrowed-never-overwrite]]). Validated for *every* entry
...
- The span *editors* (`add_related_link` / `add_supersede_banner` / `body_complement`)
  ...
  [[2026-06-02-constraint-knowledge-span-model-single-sourced]], not a new rule. The
