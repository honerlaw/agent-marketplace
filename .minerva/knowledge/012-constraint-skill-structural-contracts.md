...
The format itself (anchor grammar including the `{any_of, ignore_case}` disjunction and the 1-element-`any_of` case-insensitive idiom, the `using-minerva` self-exclusion, token-boundary matching for `minerva:<skill>`, and the reserved opaque `behavioral` namespace for the deferred behavioral runner) is documented in **`evals/README.md`** — that is the source of truth, do not restate it here.
...
- The behavioral "does this skill add value" half is a separate, deferred unit — see `.minerva/work/017-skill-contract-eval-floor/followups.md` and the reserved `behavioral` namespace.
