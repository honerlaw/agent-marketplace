...
- [[2026-06-03-decision-migration-check-read-only-entry-re-blindspot]] — the same blind spot one level up: there a corpus predating the FILENAME convention is invisible to every wiki tool, here one predating the type-FIELD convention is misread by it. Both are false-cleans, and both are why a shape check has to be tolerant of what corpora actually contain
