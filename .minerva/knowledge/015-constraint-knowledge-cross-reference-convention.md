...
`minerva:promote` maintains these links (including the reciprocal on neighbor entries) under [[016-constraint-promote-narrowed-never-overwrite]].
...
- The `## Related` block must remain the **last** section of an entry (the cross-ref span runs to EOF); the supersession banner is the only content allowed above the first `## ` header. These span boundaries are what make promote's edits safe — see [[016-constraint-promote-narrowed-never-overwrite]].
