...
**Date**: 2026-06-03
**Type**: decision
**Context**: .minerva/work/026-migrate-check (see git history if the worktree has been cleaned up)
...
[[020-decision-minerva-lint-read-only]]: it reports a migration checklist and **names**
...
- `migration_status.py` reuses the frozen detector's `ENTRY_RE` / `parse_entry` per
  ...
  [[023-constraint-wiki-edge-derivation-fence-aware]] and returns only plain primitives
