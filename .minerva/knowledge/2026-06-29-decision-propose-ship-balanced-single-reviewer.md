...
- **Extend `minerva:round-table` with a 1-agent mode and delegate to it** — a single advisory reviewer is not a 3-agent consensus panel (no quorum, no Proponent/Arbiter), so it is not the "panel mechanics" [[2026-06-10-decision-panel-mechanics-extracted-to-round-table]] keeps in round-table; forcing a degenerate 1-agent path would bloat round-table's identity and raise blast radius on a shared skill.
- **A `--verify` flag on `propose-ship-quick`** — forbidden by the never-modify-an-existing-skill rule and couples two protocols in one file; the same grounds [[2026-06-16-decision-propose-ship-quick-main-model-adjudication]] rejected "parameterize `-auto` with a mode flag."
...
## Related
- [[2026-06-16-decision-propose-ship-quick-main-model-adjudication]] — builds on
