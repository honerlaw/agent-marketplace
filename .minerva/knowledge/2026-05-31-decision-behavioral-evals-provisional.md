...
- **Don't trust the reported deltas** until the **mandatory validation spike** returns "go". Two things are unsolved: cleanly suppressing one auto-discovered skill as a control (the current control is a best-effort stub), and whether the delta is real signal vs. run-to-run noise. A no-go forces a runner replan. See `.minerva/work/2026-05-31-behavioral-skill-value-runner/followups.md`.
