...
4. **Chores in the same change**: `evals/round-table/contract.json` (anchors on the three roles, the 2/3 default, the two-vote cap, caller mode, always-convenes; `cross_surface` enabled); `propose-ship-auto`'s existing contract anchors verified still passing; all three catalog surfaces updated per [[010-constraint-minerva-skill-catalog-sync]] (plugin README row adjacent to `grill-plan`, using-minerva matrix row, root README cell token) with `propose-ship-auto`'s rows re-excerpted to name the delegation.
