...
- [solo] promote partition: 2 PROMOTE (reference: OpenAI Ads API facts; decision: openai → openai-ads refocus) / 5 MERGE INTO PROPOSAL (timeout default, empty-files refusal, /uploads docs, include vs include[], rename-threshold note) / rest DISCARD (sed mechanics, Client.instructions accessor, Docker/grep evidence) / 0 TODO (tier: default-solo row — no entry had two defensible buckets)
