...
|---|---|---|---|
| 001 | edge | → [[017-decision-knowledge-wiki-navigability-layer]] — see also | Both shape init's Routing-section detection: 001 makes the name-matching lenient (old + new dir names); 017 widened that same detection window 4→6 lines when init learned to scaffold the wiki. |
...
| 003 | edge | → [[002-bug-promote-idempotency-check-misses-old-marker]] — see also | 003's canonical-empty-state contract is built on the promote marker whose format history 002 records; 003's own body cites 002 for that history. |
| 005 | edge | → [[008-constraint-enter-worktree-absolute-paths]] — see also | The two halves of the worktree workflow's operational lore: 005 sequences `.gitignore` before `git worktree add`; 008 governs path construction after `EnterWorktree`. |
