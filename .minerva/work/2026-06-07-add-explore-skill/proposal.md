...
## Approach
**What shipped** (matches the plan below): a new pure-prose skill `plugins/minerva/skills/explore/SKILL.md`, a bounded boundary edit to `plugins/minerva/skills/propose/SKILL.md` + a `minerva:explore` anchor in `evals/propose/contract.json`, new `evals/explore/{contract,behavioral}.json`, and the three catalog edits (root `README.md`, `plugins/minerva/README.md`, `using-minerva/SKILL.md`). The durable design decision was promoted to [[2026-06-07-decision-phase-handoff-rides-observable-intake]] (phase handoffs ride an observable intake, not a self-judged predicate). All 143 minerva tests pass; the knowledge wiki lints clean.
...
- **Handoff on outcome (c) only**, via an explicit `Skill`-tool invocation of `minerva:propose "<converged direction>"`, passing the converged direction as the **inline argument** (a tool call, not narrated prose — per [[2026-05-19-constraint-skills-must-call-tools-not-prose]]).
...
- This deliberately uses **no session-scan and no self-judged "did exploration converge?" predicate** — the handoff rides propose's existing inline-arg behavior (an observable: an inline arg was passed or it wasn't), avoiding the gameable-skip failure mode of [[2026-05-31-decision-per-decision-skip-over-sizing-gate]] / [[2026-06-06-pattern-rejected-alternative-reinvented-at-runtime]].
...
- `evals/explore/contract.json` — `skill: "explore"`, frontmatter required keys, body anchors covering the load-bearing behaviors, and `cross_surface` requiring all three surfaces ([[2026-05-31-constraint-skill-structural-contracts]]). Evals live at **repo-root `evals/<skill>/`**, not under `plugins/minerva/`.
- `evals/explore/behavioral.json` — ≥1 case with a rubric (advisory per [[2026-05-31-decision-behavioral-evals-provisional]]).
- Three catalog edits ([[2026-05-21-constraint-minerva-skill-catalog-sync]]): root `README.md` minerva row, `plugins/minerva/README.md` Skills table, and `using-minerva/SKILL.md` (decision matrix — placing `explore` **before** `propose` on the commitment axis — plus a Common-scenarios entry). The using-minerva row text is copied from `explore`'s `description:` frontmatter (per the matrix source-of-truth comment).
