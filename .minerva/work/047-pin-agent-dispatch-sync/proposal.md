...
This is the same defect class as [[007-constraint-skills-must-call-tools-not-prose]] and [[049-constraint-handoffs-name-skill-tool]]: the skill text names a tool but omits the parameter that makes it behave the way the surrounding protocol assumes.
...
- **B — inline pin, no test.** Smaller diff, but nothing prevents recurrence; [[030-pattern-rejected-alternative-reinvented-at-runtime]] is precisely the observation that an unmechanized convention comes back.
