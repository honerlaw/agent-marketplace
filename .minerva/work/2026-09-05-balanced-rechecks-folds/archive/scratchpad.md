...
- [decided] promote partition: PROMOTE 2 (the decision entry written mid-work; a new pattern — a hand count is a claim until the shipped reader reproduces it), MERGE 6 review refinements + the SKILL.md-at-ceiling finding into proposal.md, DISCARD process/triage/pre-flight notes, TODO 0 (solo gate).
- [decided] TODO disposition: nothing clears or falls below the bar — #113 already tracks the parking risk; no `**Closes**` (nothing adopted, nothing resolved) (solo gate).
- [process note] the promote edit script aborted on its last replacement after the scratchpad had already been archived; the proposal rewrite and these two lines landed in a follow-up commit. Sequence file writes so the archive step runs last and only after every rewrite succeeded.
