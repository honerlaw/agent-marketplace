...
[[2026-06-03-decision-minerva-lint-read-only]]) that, behind a confirmation gate, applies
...
the [[2026-06-02-constraint-promote-narrowed-never-overwrite]] body-append-only invariant
and the [[2026-06-02-constraint-knowledge-span-model-single-sourced]] span module.
...
- **Entry edits** — only the **missing-reciprocal** family edits knowledge entries
  ...
  ([[2026-06-02-constraint-promote-narrowed-never-overwrite]]).
...
[[2026-06-02-constraint-promote-narrowed-never-overwrite]] is still the spec of record). The
...
- **Auto-fixes (deterministic only):**
  ...
  - **missing reciprocal** → insert the reciprocal `## Related` line on the neighbor;
    ...
    ([[2026-06-02-constraint-knowledge-cross-reference-convention]]). A **supersession**
...
([[2026-05-19-constraint-skills-must-call-tools-not-prose]]).

### 4. Catalog + contract + docs

- **Catalog ([[2026-05-21-constraint-minerva-skill-catalog-sync]]):** add `minerva:lint-fix`
