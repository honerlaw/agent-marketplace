...
4. A unit test for `build_record` with subagent paths is added to `tests/test_run_analyzer.py` and appended to the enumerated pytest list per [[2026-06-11-constraint-ci-test-enumeration-explicit]]. `build_record` is a pure function — no conditionality on testability.
5. No skill catalog surfaces require updates (no skill is added or removed; per [[2026-05-21-constraint-minerva-skill-catalog-sync]]).
6. `round-table/SKILL.md` remains ≤9KB after the additions per [[2026-06-11-constraint-skill-progressive-disclosure]].
