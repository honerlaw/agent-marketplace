...
| `minerva:backfill-followups` | One-time migration of an existing project's `followups.md` backlog into prioritized GitHub issues, triaging each item as open / manual / shipped / obsolete with cited evidence first. Read-only through a batched gate; an item it can't judge is filed rather than dropped. |
