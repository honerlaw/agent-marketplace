...
| `minerva:lint` | Read-only health-check for the `.minerva/knowledge/` wiki. Runs the deterministic detector for mechanical defects (index drift, broken `## Related` links, missing reciprocals) and adds LLM-judged advisory findings (orphans, contradictions, stale/superseded claims), presenting everything in `minerva:review`'s finding format. Never edits files; it reports. Durable repairs are applied by hand or by the gated fix path (Phase B.3), not by this skill. |
