---
description: Brainstorm-style proposal authoring for a new unit of work. Asks clarifying questions one at a time, proposes 2-3 approaches with tradeoffs, presents the design in sections, and writes the approved design to .minerva/work/NNN-slug/proposal.md.
---
...
- `/propose add-payments` — brainstorms a new work unit, writes `.minerva/work/NNN-add-payments/proposal.md`
...
If `.minerva/work/` already contains an entry matching the normalized slug (any `.minerva/work/NNN-<slug>/`), do **not** start a new proposal. Tell the user the existing path and suggest `/replan` if they want to course-correct an in-flight work unit.

If `.minerva/` doesn't exist yet at the project root, suggest the user run `/init` first so the directory layout and agent-file Routing section are in place. You can still write the proposal without it, but `/init` is the cleaner entry point.

## Protocol

This command mirrors the `superpowers:brainstorming` flow but writes to `.minerva/work/NNN-<slug>/proposal.md` instead of a generic spec path.

1. **Explore project context first.** Read `CLAUDE.md` / `AGENTS.md` if present, skim `.minerva/decisions/`, glance at recent `.minerva/work/NNN-*/proposal.md` files for tone and conventions. This informs the questions you'll ask.
...
1. Compute the next NNN under `.minerva/work/`:
...
2. Create `.minerva/work/NNN-<slug>/`.
