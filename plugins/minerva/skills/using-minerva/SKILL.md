...
| On the **default branch**, build a theme-grouped overview of the knowledge corpus, or check whether enough un-synthesized scope has accumulated to warrant (re)synthesis | `minerva:synthesize` (read-mostly — reports the un-synthesized-scope signal, then gated write of `overview.md`; `minerva:cleanup` runs it for you as part of reconciliation) |
