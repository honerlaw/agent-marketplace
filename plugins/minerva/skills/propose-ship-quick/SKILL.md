...
- `minerva:propose-ship-quick --cleanup-only <date-slug> --retry=N` — internal re-entry from the cleanup-gate wake-up loop. Skips phases 1–6 and re-runs Phase 7.
...
- Every decision and escalation logs one line to the work unit's `scratchpad.md` under a `## Quick decisions YYYY-MM-DD` header (`[decided]` / `[escalated to user]`).
