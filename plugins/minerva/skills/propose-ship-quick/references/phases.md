...
7. **File writes (inside the worktree).** Per `minerva:propose` steps 8–9, 11: create `.minerva/work/<date-slug>/`; write `proposal.md` and the header-only `scratchpad.md`; append the initial `## Quick decisions YYYY-MM-DD` block with the decisions from steps 3–5; `git add` + commit `chore: initialize <date-slug> work unit`.
