...
Authorized live peer sessions can message each other when the host exposes peer discovery. This file is the contract minerva applies to
...
`skills/propose/references/in-flight-check.md`. The receive half applies
...
in `skills/propose/references/in-flight-check.md` stands unchanged. Collapsing
...
3. **Bounded, filtered, non-blocking.** Volume, peer filtering and reply handling are **not
   ...
   `skills/propose/references/in-flight-check.md` §4a–4c, which owns them. Read
...
- **A reply to minerva's own pre-flight query** — a `MINERVA-BUSY` / `MINERVA-IDLE` line, or any
  ...
  `skills/propose/references/in-flight-check.md` **Step 5 and Step 6, which
...
- **At intake, before a work unit exists** — the hardcoded ask in
  `skills/propose/references/in-flight-check.md` Step 6 (resume that work / start
...
**Independent agent dispatch is delegation by design, and minerva depends on it** —
...
dispatch: instructing a subagent **this session spawned** — via the host agent API, or a host message tool to
...
`skills/propose/references/in-flight-check.md`'s "detection, not a lock":
