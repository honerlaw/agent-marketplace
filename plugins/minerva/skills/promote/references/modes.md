...
5. **TODO disposition gate.** If any entries landed in the TODO bucket, **read [references/deferral-bar.md](deferral-bar.md) now** — it is the single statement of what may be filed and where everything else goes, and it decides most of this bucket before you ask the user anything. Then run the capability probe in [references/github-issues.md](github-issues.md) step 1 if any item may become an issue, and surface the items already sorted by outlet:
   > "These forward-looking items don't belong in `## Approach`. Sorted against the deferral bar: [F] are defects small enough to fix now, which sends them back through review before promote continues; [N] clear all three conditions and would be filed as issues on `<nameWithOwner>`; [M] are standing facts about the system and would become `.minerva/knowledge/` `reference` entries; [K] I propose discarding. Documentation for behavior this diff touched isn't listed — that's being done now, not deferred. Corrections?"

   - **Absorbable defect** → not fixed here (outlet 0 of the bar). Stop Mode A before step 7 writes anything, fix the defect, log it as `- Absorbed fix: …`, and send the commits back through `minerva:review` before re-running promote. An orchestrator returns to its review phase. Nothing absorbed at promote ships unreviewed.
   - **Clears the bar** (all three conditions) → **issue path** when the probe says the repo has issues we can create: one issue per item, carrying a required `**Failure scenario**:` line, a proposed priority (`critical` / `high`), the `minerva:followup` marker label, and a back-link to this unit. **File path** otherwise — no `gh`, not authenticated, no GitHub remote, issues disabled, or creation fails for that item: append to `.minerva/work/<target>/followups.md` (create the file if missing) under a `## YYYY-MM-DD` header, one bullet per item. Both paths are specified in full in [references/github-issues.md](github-issues.md); the priority is a **proposal** that this gate and step 6 exist to correct. A second issue from one unit needs the one-line justification the bar's soft cap requires.
   - **Below the bar** → a `.minerva/knowledge/` entry of type `reference`, written as a standing fact about the system rather than as a task, or discarded. This is an ordinary PROMOTE item from step 7's point of view — same template, same add-only wiki maintenance.
...
6. **Hard gate:** do not write files until the user — or, under `--auto=<orchestrator>`, that orchestrator's adjudication — has confirmed the partition, the TODO dispositions, **and the new entry files** (each shown as a concrete diff, including its `**Summary**` and its forward `## Related` lines). There are no neighbor or `index.md` diffs to confirm — promote is add-only (see [Wiki maintenance](#wiki-maintenance-add-only)).
7. On confirmation:
   ...
   - **Author the `**Closes**` field** if this unit's diff resolves any open issues — `**Closes**: #12, #34`, directly under `**Status**:`. `minerva:ship` reads it when composing the PR body and emits one `Closes #N` line per entry, so GitHub closes them on merge; nothing else closes a `minerva:followup` issue. List only what the diff genuinely resolves: this is the one place the linkage is decided, it is authored rather than inferred, and a wrong auto-close destroys a real record where a stale-open issue merely lingers. Omit the field when nothing is closed.
     ...
     **A field that is already there is a claim, not a fact.** Intake writes `**Closes**: #NN` when the user adopted an open issue at propose time (see `skills/propose/references/issue-match.md`), which means the value predates the diff — the work may since have been replanned, narrowed, or split. Re-verify every entry you find against the actual diff and **amend or drop** the ones it no longer resolves. Dropping an entry is not a loss: it leaves the issue open, which is the cheap failure.

     **A `**Linked**` field is the other direction.** It records an issue intake matched but the user chose *not* to adopt. Nothing is promised, so nothing needs dropping — read it and **add it to `**Closes**` only if** the diff turned out to resolve it after all; otherwise leave the issue open and leave the field alone. Amend-or-drop for `**Closes**`, add-if-warranted for `**Linked**`; do not collapse the two into one rule.
