...
Each prompt is the **shared block first** (see SKILL.md § Dispatch), then — for the Arbiter only — the other agents' outputs, then the role brief.; only the layout moved the shared block to the front.
