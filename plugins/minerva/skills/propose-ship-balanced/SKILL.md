...
- At the **fixed reviewer gates** — scope check, approach selection, whole-proposal soundness, completion-verification (every run), plus mid-work divergence / replan-acceptance / replan-vs-FIX (only when triggered) — after deciding, dispatch **one** fresh-context agent via the independent reviewer operation and **wait for results**: a **Verifier** at completion, a **Skeptic** everywhere else. Arbitrate inline (fold or escalate). A **fold** gets exactly **one** fold-audit re-check by a second fresh reviewer, arbitrated strictly (a load-bearing miss escalates, never self-confirmed) — **never a third dispatch, never a panel**. Review triage, promote partition and TODO disposition are **solo**.
...
3. **Review (inline)** — minerva audit + code review (optional PR skill or independent diff reviewer); the main model triages all findings solo; replan-vs-FIX is a reviewer gate if a load-bearing finding surfaces.
