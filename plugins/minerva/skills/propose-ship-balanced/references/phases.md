...
Each reviewer gate dispatches with a gate-specific **ARTIFACT + CONTEXT** (below), `subagent_type: general-purpose`, `model: sonnet`, `run_in_background: false` (the critique is arbitrated inline in the same turn), at most **one** review dispatch per gate plus **one** fold-audit re-check when — and only when — the main model folds a Skeptic critique; never a third. CONTEXT is bounded to the run's proposal/diff, `CLAUDE.md`/`AGENTS.md`, and `.minerva/knowledge/` entries already cited this session — never a fresh corpus scan.
