...
3. **`OPEN`, auto-merge enabled** → `ScheduleWakeup` with `delaySeconds: 300`, `prompt: minerva:propose-ship-balanced --cleanup-only <NNN-slug> --retry=N`. Cap retries at 12; on exhaustion, surface manual instructions.
