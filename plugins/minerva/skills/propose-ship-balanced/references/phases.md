...
4. **Scope check — reviewer gate.** The main model decides: single unit, or decompose? Then dispatch a Skeptic. ARTIFACT = the framed scope decision + recommended pick; CONTEXT = the seed + the draft proposal. Arbitrate per verify-protocol. If the decision is "decompose", abort the run cleanly: "scope check resolved to decomposition — re-run with one sub-unit at a time."
