...
- **No PR (or PR is closed/merged)** → dispatch a fresh-context subagent via the `Agent` tool, `run_in_background: false` (its findings are presented in this same turn; a backgrounded dispatch returns only a handle and parks the review), to perform the structured code quality review (fresh eyes outperform reviewing code this context wrote) using the same finding format the minerva audit uses (severity tag + file:line + one-line description). Local-diff scope covers, at minimum:
