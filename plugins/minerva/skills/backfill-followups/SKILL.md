...
The six steps — discovery, item extraction with its atomization rule, the five dispositions and the fail-open `unsure` rule, the batched gate, filing, and the disposition ledger — live in `references/protocol.md`. **Read it in full before running any step.**
