...
**Run it on every invocation**, decoupled from worktree removal: a merge done through the GitHub UI leaves pending entries with no worktree to remove. It is cheap and silent when nothing is pending. The single exception is a repo that reconciles in CI — there the workflow owns this and cleanup stands down, because the two are unserialised rather than merely redundant (Step 0 of the reference).
