...
- **Reviewer → panel** when the Skeptic's `## Panel warranted?` section names a panel clause with evidence and the main model **cannot rule it out** on the facts. A bare "yes" with no evidence is not an event.
